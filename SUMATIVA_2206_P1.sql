...
--CASO
VAR b_fecha VARCHAR2;
EXEC :b_fecha := TO_CHAR(SYSDATE, 'DD/MM/YYYY');

DECLARE
    v_id_empleado    USUARIO_CLAVE.ID_EMP%TYPE;
    ...
    v_anios_servicio NUMBER(2);

BEGIN
    -- 1. Limpiamos la tabla primero
    /* ---------------------------------------------------------------------------
       SENTENCIA SQL 1 (DDL vía SQL Dinámico):
       Se trunca la tabla antes de iniciar para permitir ejecuciones repetitivas
       sin duplicar datos, cumpliendo con el requerimiento de limpieza inicial[cite: 16, 49].
       --------------------------------------------------------------------------- */
    EXECUTE IMMEDIATE 'TRUNCATE TABLE usuario_clave';

    -- 2. Empezamos el ciclo ordenado
    FOR r_emp IN (SELECT * FROM EMPLEADO WHERE ID_EMP BETWEEN 100 AND 320 ORDER BY id_emp ASC) LOOP
    /* ---------------------------------------------------------------------------
        SENTENCIA SQL 2 (SELECT INTO):
        Se recupera la descripción del Estado Civil para usar su primera letra
        en la generación del nombre de usuario y para la lógica del CASE en la clave.
           --------------------------------------------------------------------------- */    
        -- 3. Obtenemos el nombre del estado civil
        SELECT nombre_estado_civil 
        INTO v_est_civil
        FROM ESTADO_CIVIL
        WHERE id_estado_civil = r_emp.id_estado_civil;
        
        /* ---------------------------------------------------------------------------
           SENTENCIA PL/SQL 1 (Cálculo Aritmético):
           Cálculo paramétrico de la antigüedad utilizando la variable bind :b_fecha.
           Se usa TRUNC para obtener números enteros
           --------------------------------------------------------------------------- */
        v_anios_servicio := TRUNC( MONTHS_BETWEEN( TO_DATE(:b_fecha, 'DD/MM/YYYY'), r_emp.fecha_contrato) / 12 );
    
        -- Construimos el nombre del usuario
        v_nombre_usuario:= LOWER(SUBSTR(v_est_civil,1,1))||SUBSTR(r_emp.pnombre_emp,1,3)||LENGTH (r_emp.pnombre_emp)||'*'||
        SUBSTR(TO_CHAR(r_emp.sueldo_base),-1)||r_emp.dvrun_emp||v_anios_servicio;
        /* ---------------------------------------------------------------------------
           SENTENCIA PL/SQL 2 (Estructura Condicional IF):
           Regla de negocio: Si el empleado tiene menos de 10 años de servicio,
           se agrega una letra 'X' al final de su nombre de usuario.
           --------------------------------------------------------------------------- */
        IF v_anios_servicio < 10 THEN
            v_nombre_usuario := v_nombre_usuario || 'X';
        END IF;   
    
        -- Construimos la clave del usuario (Parte inicial)
        v_clave_usuario := SUBSTR(TO_CHAR(r_emp.numrun_emp),3,1)||(EXTRACT(YEAR FROM r_emp.fecha_nac)+2)||SUBSTR(TO_CHAR(r_emp.sueldo_base-1),-3);
    
        /* ---------------------------------------------------------------------------
           SENTENCIA PL/SQL 3 (Estructura de Control CASE):
           Se aplica la lógica de selección de letras del apellido paterno (en minúsculas)
           dependiendo del estado civil del empleado recuperado anteriormente.
           --------------------------------------------------------------------------- */
        CASE 
            -- Caso 1: Casado o Acuerdo de Unión Civil
            WHEN v_est_civil IN ('CASADO', 'ACUERDO DE UNION CIVIL') THEN 
                v_clave_usuario := v_clave_usuario || LOWER(SUBSTR(r_emp.appaterno_emp, 1, 2));
            -- Caso 2: Divorciado o Soltero
            WHEN v_est_civil IN ('DIVORCIADO', 'SOLTERO') THEN 
                v_clave_usuario := v_clave_usuario || LOWER(SUBSTR(r_emp.appaterno_emp, 1, 1)) || LOWER(SUBSTR(r_emp.appaterno_emp, -1));
            -- Caso 3: Viudo
            WHEN v_est_civil = 'VIUDO' THEN
                v_clave_usuario := v_clave_usuario || LOWER(SUBSTR(r_emp.appaterno_emp,-3 ,2));
            -- Caso 4: Separado
            WHEN v_est_civil = 'SEPARADO' THEN
                v_clave_usuario := v_clave_usuario || LOWER(SUBSTR(r_emp.appaterno_emp, -2,2 ));  
        END CASE;

        -- Parte Final Clave: ID y Fecha (usando TO_DATE en la variable bind)
        v_clave_usuario := v_clave_usuario || r_emp.id_emp || EXTRACT(MONTH FROM TO_DATE(:b_fecha, 'DD/MM/YYYY')) || EXTRACT(YEAR FROM TO_DATE(:b_fecha, 'DD/MM/YYYY'));
        
        /* ---------------------------------------------------------------------------
           SENTENCIA SQL 3 (INSERT):
           Almacenamiento final de las credenciales generadas en la tabla de resultados.
           Se concatena el nombre completo con espacios para mejor legibilidad.
           --------------------------------------------------------------------------- */
        -- 4. Guardamos los datos (usando nombres de columna correctos: appaterno_emp y apmaterno_emp)
        INSERT INTO USUARIO_CLAVE(id_emp, numrun_emp, dvrun_emp, nombre_empleado, nombre_usuario, clave_usuario) 
        VALUES (r_emp.id_emp, r_emp.numrun_emp, r_emp.dvrun_emp, r_emp.pnombre_emp||' '||r_emp.snombre_emp||' '||r_emp.appaterno_emp||' '||r_emp.apmaterno_emp, v_nombre_usuario, v_clave_usuario);

    END LOOP;
END;
/
COMMIT;
--Verificamos que se hayan insertado los datos en la tabla
